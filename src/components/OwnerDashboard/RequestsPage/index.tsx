...
import { Box, BoxConteiner, BoxInfo, BoxNotRequest, BoxStatics, ContentSearch, ContentStatics, DataContent, RegisterContent, RequestConteiner, SelectBox } from "./style";
...
export const RequestsPage = (ishow: RequestProps, ison: RequestProps) => {
  ...
  const filteRequest = request.filter((item) => {
    return item.costumer.toLowerCase().includes(search.toLowerCase())
  })
  ...
  return (
    <Conteiner>
      ...
      filteRequest.map((item,index) => {
        ...
        return (
        <BoxConteiner>
          <Box><h2> {item.id}</h2></Box>
          <Box> <h2> {item.costumer} </h2> </Box>
